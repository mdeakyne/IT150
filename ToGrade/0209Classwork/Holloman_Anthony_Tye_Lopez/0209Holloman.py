__author__ = 'Matthew'
#functions, import math, max min sqrt type
...
assert(f_add(-5,5) == 0), "Your add assert is wrong"
assert(f_sub(10,10) == 0), "Your sub assert is wrong"
assert(f_div(5,1) == 5), "Your div assert is wrong"
assert(f_mod(5,3) == 2), "Your mod assert is wrong"
assert(f_exp(2,2) == 4), "Your exp assert is wrong"
...
def calculator(a,b,operation):
    if (operation=='add'): #fill this in with correct condition:
        return a+b
    elif (operation=='sub'):
        return a-b
    elif (operation=='div'):
